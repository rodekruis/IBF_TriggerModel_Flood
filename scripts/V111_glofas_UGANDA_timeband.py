...
df_dg=pd.read_csv(path + 'input/AFFECTED_DIST_with_glofas_ABU.csv', encoding='latin-1')  
...
performance_scores.to_csv(path+ 'output/Performance_scores/uga_glofas_performance_score.csv', index=False)
 ...
 #%% QC to check manualy the result of computation of hits, misses, false_al and corr_neg for Katakwi 

district = 'Katakwi'  
station ='G5196'  

dist_stat= df_model[df_model['district']==districts]
dist_stat= dist_stat[dist_stat['station']==station]

Hits_def = dist_stat.apply(lambda x: True if x['flood']==1 and x['Q50_pred']==1 else False , axis=1)  
hits= len(Hits_def[Hits_def == True].index)
print(hits)

misses_def= dist_stat.apply(lambda x: True if x['flood']==1 and x['Q50_pred']==0 else False , axis=1) 
misses=len(Hits_def[misses_def == True].index)
print(misses)

#False alarm : first compute the number of glofas triggering events (consecutive day above discharge threshold as an event), then the false alarm are the total trigger number minus the hits
Nb_trigger = dist_stat['Q50_pred'].loc[dist_stat['Q50_pred'].shift() != dist_stat['Q50_pred']].sum()
false_al= Nb_trigger - hits
print(false_al)
     
corr_neg = misses + hits   # forcing the correct negative number to be the same than the number of observed flood events
print(corr_neg)  
