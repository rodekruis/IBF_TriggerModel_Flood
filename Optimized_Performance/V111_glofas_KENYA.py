...
#glofas_grid = path +'input/Glofas/stations_nc/upArea_Kenya.nc'
...
for station in np.unique(['G5171']):
    ...
    for i in range (0,len(nc_loc.dis)):
        ...
        if ((i+1)%120 == 0):
            extreme_discharge.append(max(discharge[(i-119):(i+1)]))
    extreme_discharge = np.asarray(extreme_discharge, dtype=np.float32)
    Gl_stations.loc[station, 'Q50'] =np.quantile(extreme_discharge,0.5)
    Gl_stations.loc[station, 'Q80'] =np.quantile(extreme_discharge,0.8)
    Gl_stations.loc[station, 'Q90'] =np.quantile(extreme_discharge,0.9)
...
# %%
df_dis1 = df_dis.sort_values(by=['KE_GVS6'],ascending=False)
df_dis1.head()

# %%
performance_scores[performance_scores['station']=='G5171']

# %%
Gl_stations[Gl_stations['station']=='G5171']
